...
Extra Credit Implemented: None

To Compile: Navigate to main project folder. Type command "./sbt compile".
To Run: After compiling, type command "./sbt run".

Some functions of Calculator program: All simple math functions of a calculator, plus binary bit
     	       	  	     	      shifting. Can also toggle individual bits in binary field to alter 
				      result, or can hide binary field if desired. Both recursive and iterative
				      implementations of math functions are available, and can switch between the two
				      by toggling the "Use Recursion" button. 
